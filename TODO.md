...
| 能力 | 命令/工具 | 状态 | 优先级 |
|------|---------|------|--------|
| P3.5-1 执行轨迹记录 | trace/ 模块：session/writer/@traced 装饰器 | ✅ 453 全量回归 | P0 |
| P3.5-2 轨迹分析 | analyzer/ 模块：stats/bottleneck/error_classify | ✅ 453 全量回归 | P0 |
| P3.5-3 经验沉淀 + 写回 | insight_engine/ + evolver/：经验提炼 + 推断链 + 风险分级写回 | ✅ 453 全量回归 | P1 |
| P3.5-4 SkillOpt 集成 | skillopt/：SDK 适配/意图识别/参数推荐 + 优雅降级 | ✅ 453 全量回归 | P1 |
| P3.5-5 离线流程触发-定时 | cron job 定时触发批量分析 | 📋 待实现（新计划） | P2 |
| P3.5-6 离线流程触发-阈值 | 轨迹数据积累到 N 条时自动触发分析 | 📋 待实现（新计划） | P2 |
| P3.5-7 离线流程触发-会话结束 | 会话结束时自动运行轻量级分析 | 📋 待实现（新计划） | P2 |

**已实现模块（P3.5-1~4）：**
- `trace/` — 会话管理 + JSONL 写入 + `@traced` 装饰器
- `analyzer/` — 统计聚合 + 瓶颈识别 + 错误归因
- `insight_engine/` — 经验提取 + 推断链生成 + 置信度计算
- `evolver/` — 风险分级 + 格式校验 + 写回执行器
- `skillopt/` — SDK 适配器 + 意图识别 + 参数推荐（优雅降级）
- CLI: `argocd-insight trace` 子命令
- SKILL.md: 可观测/轨迹/经验 触发词

**待实现（P3.5-5~7）：**
- 离线触发机制：定时 / 阈值 / 会话结束 三种触发方式
- 详情见 `docs/superpowers/plans/2026-07-02-offline-triggers.md`
