...
| P3.5-5 离线流程触发-定时 | cron job 定时触发批量分析 | ✅ 44/44 测试通过 | P2 |
| P3.5-6 离线流程触发-阈值 | 轨迹数据积累到 N 条时自动触发分析 | ✅ 44/44 测试通过 | P2 |
| P3.5-7 离线流程触发-会话结束 | 会话结束时自动运行轻量级分析 | ✅ 44/44 测试通过 | P2 |
...
- `trigger/` — 离线触发：定时（cron）/ 阈值（事件计数）/ 会话结束（atexit）
