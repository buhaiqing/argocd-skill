...
| 优化层 | **autofix**：批量修复（sync/rollback）；**impact**：变更影响分析；**batch**：批量操作（sync/rollback/refresh 并发执行）；**scaffold**：配置模板（4-tier 模型快速生成） | **patrol**：定时巡检；**audit**：部署审计 |

### 缺口池（下一步优先级）

| 缺口 | 来源 | 优先级 | 目标版本 |
|------|------|--------|----------|
| P3-5 定时巡检（patrol） | 主动发现问题 | P2 | v0.5.0 |
| P3-6 部署审计（audit） | 合规与追溯 | P2 | v0.5.0 |
